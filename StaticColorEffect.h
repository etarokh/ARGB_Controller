#pragma once

#include <Arduino.h>
#include <FastLED.h>

#include "DecorativeEffect.h"
#include "LedManager.h"

class StaticColorEffect : public DecorativeEffect {
public:
  void begin(LedManager* ledManager) override;

  void update() override;

  void reset() override;

  void setColor(const CRGB& color) override;

  void nextColor();

  void setColorIndex(
    uint8_t index
  );

  uint8_t getColorIndex() const;

  const char* getColorName() const;

  static constexpr uint8_t getColorCount()
  {
    return COLOR_COUNT;
  }

private:
  static constexpr uint8_t COLOR_COUNT = 30;

  uint8_t colorIndex = 0;

  void applyCurrentColor();

  LedManager* leds = nullptr;

  CRGB effectColor = CRGB::Black;

  bool needsRender = true;
};