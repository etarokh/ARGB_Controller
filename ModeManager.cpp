...
void ModeManager::update() {
  ...
  if (
    ...
    if (alertOverrideActive) {
      ...
      {
        monitorManager->forceRender();

        Serial.println(
          "Monitor resumed after Alert"
        );
      }
    }
  }
...
void ModeManager::setMode(
  ...
  if (
    ...
    monitorManager->resetToFirstPage();
  }
  ...
  if (
    ...
    Serial.println(
      monitorManager->getMonitorPageName()
    );
  }
}
...
void ModeManager::applyMode() {
  ...
  if (monitorManager != nullptr) {
    monitorManager->setEnabled(false);
  }
